...
class Button2 extends Phaser.GameObjects.Image {
    constructor(scene, x, y, texture, config) {
        super(scene, x, y, texture)
        scene.add.existing(this)
...
class FillBar extends Phaser.GameObjects.Container {
    constructor(scene, x, y, width, height, startPercent) {
        ...
        this.topBar = scene.add.image(this.backBar.getLeftCenter().x + 30, -3, "ui-fillbar-top")
        .setOrigin(0, 0.5)
    ...
    update(percent, y) {
        this.topBar.setScale(percent, y)
    }
}
...
class Bar extends Phaser.GameObjects.Container {
    constructor(scene, x, y, width, height, startPercent) {
        super(scene, x, y)
        scene.add.existing(this)
        this.backBar = scene.add.image(0, 0, "game-bar-bottom")
        this.topBar = scene.add.image(this.backBar.getLeftCenter().x + 20, -5, "game-bar-top")
        .setOrigin(0, 0.5)
...
class BarMinigame extends Phaser.GameObjects.Container {
    constructor(scene, x, y, width, height, startPercent) {
        super(scene, x, y)
        scene.add.existing(this)
        this.backBar = scene.add.image(0, 0, "minigame-scene-bar-bottom")
        this.topBar = scene.add.image(-4,  this.backBar.getBottomCenter().y-15 , "minigame-scene-bar-top")
        .setOrigin(0.5, 1)
    ...
    update(percent) {
        this.topBar.setScale(this.scaleX, percent)
